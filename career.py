...
def get_current_mission() -> str | None:
    ...
    generated_dir = Path("missions/greenlogistics/generated")
    if not generated_dir.exists():
        return None
    files = sorted(generated_dir.glob("greenlogistics-*.json"))
    if not files:
        return None
    return files[-1].stem
...
def cmd_start():
    ...
    mission_file = Path(f"missions/greenlogistics/generated/{mission_id}.json")
...
def cmd_status():
    """Affiche le niveau, l'XP et l'historique."""
    progress = load_progress()
    career = load_career()
    print(f"\n👤 Joueur : {progress.player.name}")
    print(f"Niveau : {progress.player.current_level}")
    print(f"XP : {career.get('xp', 0)}")
    print(f"Missions validées : {len(career.get('missions_completed', []))}")
    print("\nCompétences :")
...
def cmd_regenerate():
    ...
    progress = load_progress()
    # Supprime la dernière mission générée
    mission_id = get_current_mission()
    if mission_id:
        mission_file = Path(f"missions/greenlogistics/generated/{mission_id}.json")
    ...
    new_id = f"{mission_id or 'greenlogistics-001'}-regen"
    mission = generate_mission(llm, progress, new_id)

    out_dir = Path("missions/greenlogistics/generated")
...
def main():
    parser = argparse.ArgumentParser(description="GreenLogistics DevOps RPG")
    parser.add_argument(
        "--start",
        action="store_true",
