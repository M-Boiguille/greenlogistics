...
def get_pr_files(pr_number):
    ...
    result = {}
    for file in files:
        # On ne lit que les ajouts/modifications complètes pour éviter les gros patches
...
def extract_mission_id(body):
    ...
    match = re.search(r"Mission (greenlogistics-\d{3})", body)
    if match:
...
def approve_pr(pr_number):
    """Approuve la PR si le Lead a validé."""
    if not REPO or not GITHUB_TOKEN:
        print("GITHUB_REPOSITORY ou GITHUB_TOKEN manquant.")
        return None
    ...
    data = {
        "body": "Mission approuvée par le Lead IA.",
        "event": "APPROVE",
    }
    response = requests.post(url, headers=headers, json=data)
    response.raise_for_status()
...
def main():
    ...
    if not mission_id:
        ...
        # On essaie de deviner depuis le titre du dernier fichier de mission
